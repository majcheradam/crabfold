...
export default function AgentOverviewPage() {
  ...
  const handleCopyClone = () => {
    navigator.clipboard.writeText(
      `npx crabfold clone ${params.agentSlug} ./my-agent`
    );
    setCopied(true);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-4 border border-border p-4">
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-2 text-xs leading-relaxed text-muted-foreground">
            ...
            <p>
              The agent uses a{" "}
              <span className="text-foreground">ReAct reasoning loop</span> with
              Claude Sonnet to classify incoming GitHub issues, apply labels,
              and assign owners. A{" "}
              <span className="text-foreground">Heartbeat</span> schedule polls
              for unassigned issues every 5 minutes as a safety net.
            </p>
          </div>
        </div>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <Link href={`/${params.username}/${params.agentSlug}/deploy`}>
